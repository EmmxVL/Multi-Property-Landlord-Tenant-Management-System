...
try {
    ...
    // 2️⃣ Active Landlords (Role ID = 1 and 'approved')
    $stmt = $db->query("
        SELECT COUNT(u.user_id) AS active_landlords
        FROM user_tbl u
        JOIN user_role_tbl ur ON u.user_id = ur.user_id
        WHERE ur.role_id = 1 AND u.status = 'approved'
    ");
    $activeLandlords = $stmt->fetch(PDO::FETCH_ASSOC)['active_landlords'] ?? 0;

    // 3️⃣ Active Tenants (Role ID = 2 and 'approved' and Active Lease)
    $stmt = $db->query("
        SELECT COUNT(DISTINCT l.user_id) AS active_tenants
        FROM lease_tbl l
        JOIN user_tbl u ON l.user_id = u.user_id
        WHERE u.status = 'approved' AND l.lease_status = 'Active'
    ");
    ...
    $totalProperties = $activeLandlords = $activeTenants = $pendingApplications = 0;
    // Optional: log $e->getMessage();
}

try {
    // *** MODIFIED: Only fetch 'approved' landlords for this list ***
    $stmt = $db->prepare("
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-slate-200 property-card">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-green-600 mt-1">Approved accounts</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-slate-200 property-card">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-slate-800 mt-1"><?= $pendingApplications ?></p>
                <p class="text-xs text-purple-600 mt-1">Awaiting approval</p>
            </div>
  ...
  <section class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
      <div class="flex items-center space-x-3 mb-4 sm:mb-0">
        ...
        <div>
          ...
          <p class="text-slate-600 text-sm">Manage approved landlord accounts</p>
        </div>
      </div>

      <div class="flex space-x-3">
        <!-- *** NEW: Manage Applications Button *** -->
        <a href="../manageApplications.php" class="bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-4 rounded-lg flex items-center space-x-2 transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>Manage Applications (<?= $pendingApplications ?>)</span>
        </a>
      ...
      <table class="w-full text-sm" id="landlordsTable">
          ...
          <tr
              ...
              <td class="py-4 px-4">
                <!-- *** MODIFIED: Status is now dynamic *** -->
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium 
                    <?php if ($landlord['status'] === 'approved'): ?>
                        bg-green-100 text-green-800
                    <?php elseif ($landlord['status'] === 'pending'): ?>
                        bg-yellow-100 text-yellow-800
                    <?php else: ?>
                        bg-red-100 text-red-800
                    <?php endif; ?>
                ">
                    <?= ucfirst(htmlspecialchars($landlord['status'])); ?>
                </span>
              </td>
              ...
              <div class="flex justify-center space-x-2">
                  ...
                  <!-- *** MODIFIED: Form points to new handler *** -->
                  <form id="deleteForm<?= htmlspecialchars($landlord['user_id']); ?>" method="POST"
                  action="../admin.php" class="hidden">
      ...
      <button id="empty-add-landlord-btn"
   ...
   <!-- Add Landlord Modal -->
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95 opacity-0 modal-content">
            ...
            <input type="hidden" name="action" value="add_landlord">
            ...
            <div class="p-8">
                <div class="space-y-6">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Philippine mobile number format (09...)</p>
                    </div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        Landlord will be prompted to change this on first login
                    </p>
                    </div>
                </div>
                ...
                <div class="mt-6 bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg">
                    ...
                    <div class="ml-3">
                        ...
                        This account will be created as 'Approved' and will be able to log in immediately.
                        </p>
                    </div>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const emptyOpenBtn = document.getElementById('empty-add-landlord-btn'); // Button from empty state
  ...
  if (emptyOpenBtn) emptyOpenBtn.addEventListener('click', showModal); // Listen to the empty state button too
  ...
  modal.addEventListener('click', (e) => {
    ...
    if (toggleBtn && passwordInput) {
        toggleBtn.addEventListener('click', () => {
            ...
            // Optional: Change eye icon (requires more complex SVG swapping)
        });
    }
});
</script>
