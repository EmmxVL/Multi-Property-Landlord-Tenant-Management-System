...
if (isset($_SESSION['user_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == 'Landlord') {
        header("Location: PHP/dashboard/landlord_dashboard.php");
        exit();
    } elseif ($_SESSION['role'] == 'Tenant') {
        header("Location: PHP/dashboard/tenant_dashboard.php");
        exit();
    }
}
    ...
    <div class="w-full max-w-md">
      <div class="bg-white/80 backdrop-blur-md rounded-3xl shadow-lg border border-slate-200 p-8 animate-rise">
        ...
        <h2 class="text-2xl font-bold text-center text-blue-900 mb-2">
          User <span class="text-blue-700">Login</span>
        </h2>
        <p class="text-center text-slate-500 text-sm mb-6">Sign in as a Landlord or Tenant.</p>

        <form method="POST" action="PHP/login_user.php" class="space-y-4">
          <div>
          ...
          <button type="submit"
            ...
            <div>
                <p class="text-sm text-slate-600">
                    ...
                    <a href="PHP/signup.php" class="font-semibold text-blue-600 hover:text-blue-700 hover:underline">
                        Apply Here
                    </a>
                </p>
            </div>
